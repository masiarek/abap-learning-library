...
- **ABAP cheat sheets** — SAP's own runnable examples for modern syntax, at [SAP-samples/abap-cheat-sheets](https://github.com/SAP-samples/abap-cheat-sheets). The closest thing to this library published by SAP, and a good place to check a construct quickly. One sheet to read end to end rather than look things up in: [Excursion Down to Bits and Bytes](https://github.com/SAP-samples/abap-cheat-sheets/blob/main/09_Bits_and_Bytes.md) puts the same four bytes, `32003700`, into a `TYPE i`, a `TYPE c LENGTH 2` and a `TYPE x LENGTH 4`, and the debugger reads them as 3604530, `27` and `32003700` — then `ASSIGN … CASTING` changes the reading without changing a byte, which is the difference between casting and the [conversion rules](../conversion_and_comparison_rules/README.md) in one screenful, and the two-bytes-a-character text it relies on is where [Unicode and code pages](../unicode_and_code_pages/README.md) starts.
